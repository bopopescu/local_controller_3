...
class Modbus_Class(object):
   ...
   def generate_properties(self):
       return_value = []
       return_value.append(self.generate_property(name= "BASIC_STATS",redis_key = self.basic_key))
       for i in self.address_list:
            return_value.append(self.generate_property(name= "REMOTE_STATS_"+str(i),redis_key = self.remote_key+str(i)))  
       return return_value

   def generate_property(self,name,redis_key):           
       temp = {}
       temp["name"] = name
       temp["redis_key"] = redis_key       
       temp["repeat_time"] = 0 #seconds which is one hour
       temp["error_time"]  = 2*24*3600# two days
...
class Process_Class(object):
   ...
   def generate_properties(self):
      ...
      for i in self.process_list:
         ...
         error_file = "/tmp/"+temp_name[-0]
         temp["name"] = name
         temp["repeat_time"] = 3600 #seconds which is one hour
         temp["error_time"]  = 24*3600*7 # one week timeout
         temp["current_time"] = time.time()
         temp["status"] = "GREEN"
...
class Sub_Process_Class(Process_Class):
   ...
   def __init__(self,cf,parent_process,label, sub_process_list):
       super(Sub_Process_Class,self).__init__(cf,label,sub_process_list)
       self.parent_process = parent_process
       for i in self.properties:
          i["parent_process"] = parent_process
