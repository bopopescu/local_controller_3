...
class Irrigation_Control_Basic:
   ...
   def construct_chains( self , cf ):
       ...
       cf.insert.verify_function_terminate(  reset_event = "IR_D_END_IRRIGATION",
                                             reset_event_data=None,
       ...
       cf.insert.verify_function_terminate(  reset_event = "IR_D_END_IRRIGATION",
                                             reset_event_data=None,
                                             function = self.measure_current)
       cf.insert.one_step( self.turn_on_io) 
       cf.insert.reset()
   ...
   def update_control_variables( self ):
       ...
       #self.redis_handle.hset( "CONTROL_VARIABLES","schedule_step", self.json_object["schedule_step"] )
   ...
   def measure_current( self, cf_handle, chainObj, parameters, event ):
       if event["name"] == "INIT":
          return True
           ...
           return_value = True
           current =  self.io_control.measure_valve_current()
           if current > 24:
               ...
               return_value = False
       print("measure current",current,return_value)
       return return_value
   ...
   def start( self, cf_handle, chainObj, parameters, event):
       ...
       if "elasped_time" not in self.json_object:
       ...
       self.io_control.load_duration_counters( self.json_object["run_time"]  + 5 )
       self.io_control.turn_on_master_valves()
       self.io_control.turn_on_valve(self.json_object["io_setup"])
       self.elasped_time = 0
