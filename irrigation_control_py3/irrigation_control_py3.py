...
class Irrigation_Control:
   ...
   def __init__( self,   io_control, redis_handle, alarm_queue, app_files, sys_files ):
       self.cf = cf
       self.io_control = io_contol
       self.redis_hanle = redis_handle
   ...
   def construct_chains( self , cf ):
       cf.define_chain("start_irrigation_step", True ) # tested
       cf.insert_link( "link_1,  "Verify_Terminate_Event",   [ sprinkler_queue.load_irrigation_cell, XXX Termination _event ] )
       cf.insert_link( "link_2",  "Verify_Terminate_Event",   [ sprinkler_element.start] , xxxxx Termination Event )
       cf.insert_link( "link_3",  "WaitTime",       [ 1,0,0,0] ) # wait 1 seconds
       cf.insert_link( "link_4",  "One_Step",       [ monitor.measure_current_a ] )
       cf.insert_link( "link_5",  "Verify_Terminate_Event",   [ sprinkler_element.check_current, xxxxxxxxxxxx ] )
       cf.insert_link( "link_6",  "Enable_Chain",   [["monitor_irrigation_cell","monitor_current_sub" ]])
       cf.insert_link( "link_7",  "Terminate", [] )
       ...
       cf.insert_link( "link_3",  ""Verify_Terminate_Event"",       [ sprinkler_element.check_current , xxxxxx] )
       cf.insert_link( "link_4",  "Reset",          [] )


       cf.define_chain("monitor_irrigation_step", False ) #Tested
       ...
       cf.insert_link( "link_4",  "SendEvent",      ["xxxx Termanination "] ) 
   ...
   def construct_clusters( self, cluster, cluster_id, state_id ):
       cluster.define_state( cluster_id, [state_id[0]],["start_irrigation_step"]  )
