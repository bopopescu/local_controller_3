...
class PI_Web_Server(object):
   ...
   def __init__(self , name, redis_handle, app_files, sys_files,gm, users ):
       app         = Flask(name) 
       auth = HTTPDigestAuth()
...
if __name__ == "__main__":
   ...
   redis_handle = redis.StrictRedis(
        host=data_server_ip, port=data_server_port, db=0)

   sys_files = load_files_py3.SYS_FILES(redis_handle)
   app_files = load_files_py3.APP_FILES(redis_handle)
   users = {"admin":"password"}
   pi_web_server = PI_Web_Server(__name__, redis_handle, app_files, sys_files,gm, users )
